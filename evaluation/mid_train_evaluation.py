...
# stylegan_metrics_results = {'results': {'fvd2048_10f': 4845.533858185447}, 'metric': 'fvd2048_10f', 'total_time': 79.95496463775635, 'total_time_str': '1m 20s', 'num_gpus': 1, 'snapshot_pkl': None, 'timestamp': 1763237845.608205} 
    ...
    def process_checkpoint(self, name: str | None = None, save_results:bool = True, save_dir: Path | None = None, latent_dir=None):
        decoded_videos_dirs = self.decode_checkpoint_latents(
            self._name_to_latent_path(name) if latent_dir is None else latent_dir
        )
...
class MultiRunAndStepETPWrapper(EvaluateTrainProcess):
    ...
    '''
    def __init__(self, cfg, ckpt_selection_fn: callable):
        self.run_dirs = [Path(rd) for rd in cfg.run_dirs]
        self.steps = cfg.n_inference_steps
    ...
    def _prepare_dataloaders(self, cfg):
    ...
    def _get_ckpt_etp_results(self):
        ckpts = {}
        for run_dir in self.cfg.run_dirs:
    ...
    def sample_latents_from_ckpt(self, ckpt_dict, app='', kwargs={}):
        ...
        sample_dl = self._prepare_dataloaders(run_cfg)
        dummy_data = next(iter(sample_dl.values()))[0].dataset[0]
        ...
        latents_dir = sample_latents_from_model(
            ...
            step=ckpt_dict['step'],
            device=self.device,
            samples_dir=self.output_dir / ckpt_dict['wandb_name'],
            out_name=f"epoch={ckpt_dict['epoch']}-step={ckpt_dict['step']}-latents{app}",
            kwargs=kwargs
        )
    ...
    def sample_all_latents(self):
        ...
        for wandb_name, ckpts in self.ckpts.items():
            for ckpt_dict in ckpts:
                for steps in self.cfg.n_inference_steps:
                    latents_dir = self.sample_latents_from_ckpt(
                        ckpt_dict, 
                        app=f"-inf_steps={steps}", 
                        kwargs={'model_sample_kwargs': {'steps': steps}}
                    )
                    latents_dirs.append(latents_dir)

        return latents_dirs

    def _results_to_rows(self, name, task, inf_steps, metric_results):
        rows = [
            {'checkpoint': name, 'task': task, 'type': k, 'inf_steps': inf_steps,
             **v.get('stylegan_results', {}), 
             **{ pm['metric']: pm['mean'] for pm in v.get('summary', []) }
            } 
                for k, v in metric_results.items()
    ...
    def decode_checkpoint_latents(self, latents_dir: Path):
        super().decode_checkpoint_latents(latents_dir)

    def process_checkpoint(self, name=None, save_results = True, save_dir = None, latent_dir=None):
        rows = super().process_checkpoint(name, save_results, save_dir, latent_dir)
        
        str_ld = str(latent_dir)
        if 'inf_steps=' in str_ld:
            inf_steps = int(re.search(r"inf_steps=(\d+)", str_ld).group(1))
    
            rows = [{**r, 'inf_steps': inf_steps} for r in rows]
        return rows


    def main(self):

        latents_dirs = self.sample_all_latents()

        rows = []
        for latent_dir in latents_dirs:
        ...
        results_df = pd.DataFrame(rows)
        save_dir = self.output_dir / "multirun_and_step_results.csv"
    ...
    def process_checkpoints(self, save_results: bool = True, save_dir: Path | None = None, delete_latents_after: bool = False):
        ...
        self.results_df = results_df
        return results_df
#TODO FINISH ME. CFG YAML? LOGIC SOUND?